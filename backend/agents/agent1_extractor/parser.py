...
def _extract_pdf_llamaparse(pdf_path: str) -> tuple[str, str]:
    ...
    try:
        parser = llama_parse.LlamaParse(
            api_key=api_key,
            result_type="markdown",   # richer than "text" for complex layouts
            verbose=False,
        )
...
def _extract_pdf_pymupdf(pdf_path: str) -> tuple[str, str]:
    """Extract text from a PDF using PyMuPDF (local fallback)."""
    try:
        import fitz  # PyMuPDF
        ...
        doc.close()
        return "\n\n".join(pages), "pymupdf_fallback"
    except ImportError:
        raise ImportError(
            "PyMuPDF is not installed. Install it with: pip install PyMuPDF\n"
            "Or set LLAMA_CLOUD_API_KEY so LlamaParse can be used instead."
        )
...
def parse_cv(
    ...
    """
    from agents.agent1_extractor.prompt import (
        SYSTEM_PROMPT,
        build_retry_prompt,
        build_user_prompt,
    )
    from agents.agent1_extractor.schema import CVExtraction
